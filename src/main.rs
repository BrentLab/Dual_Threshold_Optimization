...
use dual_threshold_optimization::read::{read_feature_list_from_file, read_ranked_feature_list_from_csv};
...
fn main() {
    ...
    eprintln!(
        "Multi-node mode: {}",
        if cli.multi_node { "enabled" } else { "disabled" }
    );

    let ranked_feature_list1 = read_ranked_feature_list_from_csv(
        cli.ranked_list1.to_str().expect("Invalid file path for input1"),
    );
    let ranked_feature_list2 = read_ranked_feature_list_from_csv(
        cli.ranked_list2.to_str().expect("Invalid file path for input2"),
    );
    ...
    let background = if let Some(background_file) = cli.background {
        Some(read_feature_list_from_file(
            background_file.to_str().expect("Invalid file path for background"),
        ))
    } else {
        None
    };
    ...
    let mut tasks: Vec<Task> = vec![Task { id: 0, permute: false }];
...
//         eprintln!("Background list: {}", background.display());
//     } else {
//         eprintln!("No background list provided.");
//     }
//     eprintln!("Permutations: {}", cli.permutations);
//     eprintln!("Threads: {}", threads);
//     eprintln!(
...
//     // Move `background` outside of `if let` for scope purposes
//     let background = if let Some(background_file) = cli.background {
//         Some(read_feature_list_from_file(
//             background_file.to_str().expect("Invalid file path for background"),
//         ))
...
//     // This computes the population size, and also checks that the rank lists
//     // and background, if it is passed, fit expectations
//     let population_size = compute_population_size(
//         &ranked_feature_list1,
//         &ranked_feature_list2,
//         background.as_ref());

//     // Create the tasks: one unpermuted and the rest permuted. If cli.permutations is
//     // 0, then only the unpermuted task is created.
//     let mut tasks: Vec<Task> = vec![Task { id: 0, permute: false }];
//     tasks.extend((1..=cli.permutations).map(|id| Task { id, permute: true }));
...
//         if mpi::initialize().unwrap().world().rank() == 0 {
